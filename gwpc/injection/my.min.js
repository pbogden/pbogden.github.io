...
point[1]=y1-y0;x0=x1;y0=y1}}function noop(){}if(typeof define==="function"&&define.amd)define(topojson);else if(typeof module==="object"&&module.exports)module.exports=topojson;else this.topojson=topojson}();var width=960,height=500;var projection=d3.geo.albers();var color=d3.scale.linear().range(["yellow","red"]);var path=d3.geo.path().projection(projection);var svg=d3.select("body").append("div").attr("id","mymap").style("width",width+"px").style("height",height+"px").append("svg").attr("width",width).attr("height",height);d3.json("data.json",function(err,data){if(err)return console.log("error reading data",err);var counties=topojson.feature(data.ok,data.ok.objects.ok);var ok=topojson.merge(data.ok,data.ok.objects.ok.geometries);projection.scale(1).translate([0,0]);var b=path.bounds(ok),s=.95/Math.max((b[1][0]-b[0][0])/width,(b[1][1]-b[0][1])/height),t=[(width-s*(b[1][0]+b[0][0]))/2,(height-s*(b[1][1]+b[0][1]))/2];projection.scale(s).translate(t);var locations=data.data.map(function(d){return d3.merge([projection([d.lon,d.lat]),[d.depth]])});color.domain(d3.extent(data.data.map(function(d){return d.depth})));svg.append("path").datum(ok).attr("class","state").attr("d",path);svg.append("path").datum(counties).attr("class","county").attr("d",path);var format=d3.format(",d");var label=format(locations.length)+" unique locations<br>";var info=d3.select("body").append("div").attr("class","info").html(label);svg.selectAll(".dot").data(locations).enter().append("circle").attr("class","dot").attr("cx",function(d){return d[0]}).attr("cy",function(d,i){return d[1]>=0?d[1]:null}).attr("r",1).style("fill",function(d){return color(d[2])}).on("mouseover",function(d,i){d3.select(this).style("fill","red");d3.select(this).attr("r",5);info.html(label+"depth: "+d[2]+"<br>"+mousemoved())}).on("mouseout",function(d,i){d3.select(this).style("fill",color(d[2]));d3.select(this).attr("r",1);info.html(label)});function mousemoved(){return formatLocation(projection.invert(d3.mouse(svg.node())))}});function formatLocation(p){var format=d3.format(".3f");return(p[1]<0?format(-p[1])+"°S":format(p[1])+"°N")+" "+(p[0]<0?format(-p[0])+"°W":format(p[0])+"°E")}